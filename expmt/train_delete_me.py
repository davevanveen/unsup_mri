...
def train(args):
    ...
    if not args.trial_id:
        args.trial_id = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
    ...
    for file_id in file_id_list:
        ...
        ss, xx, yy = ksp_orig.shape[0], ksp_orig.shape[1], ksp_orig.shape[2]
        #ksp_orig = ksp_orig[:int(ss/2), :, :]
